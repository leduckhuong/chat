...
if (langIcon) {
    ...
    const updateLangIcon = (code, src) => {
        ...
        const html = `<div class="min-w-8 h-8 mr-1" style="background: url('${src}') top center / cover no-repeat;"></div>${lang.code}`;
        langIcon.html(html);
    };
    ...
    langOptions.each((index, lang) => {
        let src = lang.firstElementChild.currentSrc;
        let code = $(lang).data('code');
        code = checkLang(code) ? code : 'en';
        $(lang).on('click',() => {
            if(userLang!==code) {
    ...
    if(!userLang) {
        userLang = 'en';
        updateLangIcon(userLang, '/imgs/english.png');
    };
